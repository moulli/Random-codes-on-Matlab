function [Wnet, architecture] = f_RpropCasCorTrain(Xtrain, ytrain, lambda, pool, time, sen)
...
% Hippolyte MOULLE
    ...
    W = f_RpropResilientBasic(Xtrain, ytrain, winit, lambda, sen);
    ...
    [theta, type] = f_RpropMaxCovariance(Xtrain, ytrain, W, pool, sen, 0);
...
%     W = 0.012 * (rand(size(W, 1), size(W, 2) + 1)-0.5);
    ...
    W = f_RpropResilientBasic(Xtrain, ytrain, W, lambda, sen);
    ...
    while toc < time
        ...
        [theta, type] = f_RpropMaxCovariance(Xtrain, ytrain, W, pool, sen, nbsib);
        % Freezing its weights and its location in the net architecture:
        Wnet{end+1} = theta;
